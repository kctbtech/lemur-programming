        ...
        if "echo" in data[i] and not "math" in data[i]:
            if "end" in data[i]:
                if "seperator" in data[i]:
                    if data[i+1][0] == "&" and '+' not in data[i+1] and '-' not in data[i+1] and '/' not in data[i+1] and '*' not in data[i+1] and '%' not in data[i+1] and '(' not in data[i+1] and ')' not in data[i+1]:
                        print(str(variables[data[i+1][1:]]) + " ", end = '')
                    else:
                        print(str(data[i+1]) + " ", end = '')
                elif data[i+1][0] == "&" and '+' not in data[i+1] and '-' not in data[i+1] and '/' not in data[i+1] and '*' not in data[i+1] and '%' not in data[i+1] and '(' not in data[i+1] and ')' not in data[i+1]:
                    print(str(variables[data[i+1][1:]]), end = '')
                else:
                    print(str(data[i+1]), end = '')
            ...
            outcome = a + b
            i+=4
        elif data[i] == "get_remainder":
            if data[i+1][0] == "&" and '+' not in data[i+1] and '-' not in data[i+1] and '/' not in data[i+1] and '*' not in data[i+1] and '%' not in data[i+1] and '(' not in data[i+1] and ')' not in data[i+1]:
                firstnum = variables[data[i+1][1:]]
            else:
                firstnum = calculate(data[i+1])
            if data[i+3][0] == "&" and '+' not in data[i+3] and '-' not in data[i+3] and '/' not in data[i+3] and '*' not in data[i+3] and '%' not in data[i+3] and '(' not in data[i+3] and ')' not in data[i+3]:
                secondnum = variables[data[i+3][1:]]
            else:
                secondnum = calculate(data[i+3])
            remainder = firstnum % secondnum
            i+=4
        elif data[i] == "assign_remainder":
            assignDATA(data[i+1], remainder)
            i+=2
        elif data[i] == "return_remainder":
            print(remainder)
            i+=1
        elif data[i] == "replace":
            if data[i+3][0] == "&":
